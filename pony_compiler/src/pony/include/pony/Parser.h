...
class Parser {
 ...
 private:
  ...
  std::unique_ptr<VarDeclExprAST> parseDeclaration() {
    ...
    //       variable name
    if (lexer.getCurToken() != tok_identifier)
      return parseError<VarDeclExprAST>("identifier", "after 'var'");
    id = std::string(lexer.getId());
    lexer.consume(tok_identifier);
    ...
    if (lexer.getCurToken() == '<') {
      if (type)
        return parseError<VarDeclExprAST>("=", "after typed variable name");
      type = parseType();
      if (!type) return nullptr;
    }
    ...
    if (lexer.getCurToken() != '=')
      return parseError<VarDeclExprAST>("=", "in variable declaration");
    lexer.consume(Token('='));
    auto expr = parseExpression();
    if (!expr) return nullptr;
  ...
  std::unique_ptr<VarType> parseType() {
    ...
    if (lexer.getCurToken() != tok_number)
      return parseError<VarType>("dimension", "in tensor type");

    while (true) {
      double dimension = lexer.getValue();
      if (!std::isfinite(dimension) || dimension < 0 ||
          std::floor(dimension) != dimension ||
          dimension >
              static_cast<double>(std::numeric_limits<int64_t>::max()))
        return parseError<VarType>("non-negative integer",
                                   "for tensor dimension");

      type->shape.push_back(static_cast<int64_t>(dimension));
      lexer.consume(tok_number);

      if (lexer.getCurToken() == close) break;
      if (lexer.getCurToken() != ',')
        return parseError<VarType>(",", "between tensor dimensions");
      lexer.consume(Token(','));
      if (lexer.getCurToken() != tok_number)
        return parseError<VarType>("dimension", "after ',' in tensor type");
    }
  ...
  std::unique_ptr<ExprAST> parseIdentifierExpr() {
    ...
    if (lexer.getCurToken() != ')') {
      while (true) {
        ...
        lexer.consume(Token(','));
        if (lexer.getCurToken() == ')')
          return parseError<ExprAST>("expression", "after ',' in argument list");
      }
    }
    ...
    if (name == "print") {
      if (args.size() != 1)
        return parseError<ExprAST>("one argument", "in print call");
  ...
  std::unique_ptr<ExprAST> parseBinOpRHS(int exprPrec,
    ...
    while (true) {
      ...
      auto binOpLoc = lexer.getLastLocation();
      ...
      lhs = std::make_unique<BinaryExprAST>(std::move(binOpLoc), binOp,
