...
struct SimplifyRedundantTranspose : public mlir::OpRewritePattern<TransposeOp> {
  ...
  matchAndRewrite(TransposeOp op,
    ...
    // Hint: For op, there is a function: op.getOperand(), it returns the
    // parameter of a TransposeOp and its type is mlir::Value.
    ...
    //       If this value is the result of an operation of type OpTy, return
    //       the operation that defines it.
    auto definingTranspose = input.getDefiningOp<TransposeOp>();
    if (!definingTranspose)
      return failure();

    // step 3: Otherwise, we have a redundant transpose. Use the rewriter to
    // remove redundancy.
    // Hint: For mlir::PatternRewriter, there is a function you may use to
    // remove redundancy:
    //       void replaceOp (mlir::Operation *op, mlir::ValueRange newValues)
    //       The first argument will be replaced by the second argument.
    rewriter.replaceOp(op, definingTranspose.getOperand());
    return success();
  }
};
