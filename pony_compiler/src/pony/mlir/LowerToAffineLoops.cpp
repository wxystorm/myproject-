...
struct GemmOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    // The three loops iterate over result row, result column, and reduction
    // dimension respectively.
    auto lhsType = operands[0].getType().cast<MemRefType>();
    auto rhsType = operands[1].getType().cast<MemRefType>();
    if (lhsType.getShape()[1] != rhsType.getShape()[0])
      return rewriter.notifyMatchFailure(op, [](Diagnostic &diag) {
        diag << "expected gemm inner dimensions to match";
      });
    upperBounds[0] = tensorType.getShape()[0];
    upperBounds[1] = tensorType.getShape()[1];
    upperBounds[2] = lhsType.getShape()[1];

    SmallVector<int64_t, 2> initLowerBounds(2, /*Value=*/0);
    SmallVector<int64_t, 2> initUpperBounds;
    initUpperBounds.push_back(tensorType.getShape()[0]);
    initUpperBounds.push_back(tensorType.getShape()[1]);
    SmallVector<int64_t, 2> initSteps(2, /*Value=*/1);
    buildAffineLoopNest(
        rewriter, loc, initLowerBounds, initUpperBounds, initSteps,
        [&](OpBuilder &nestedBuilder, Location loc, ValueRange ivs) {
          auto zero = nestedBuilder.create<arith::ConstantOp>(
              loc, nestedBuilder.getF64FloatAttr(0.0));
          nestedBuilder.create<AffineStoreOp>(loc, zero, alloc, ivs);
        });
    ...
    buildAffineLoopNest(
        ...
        [&](OpBuilder &nestedBuilder, Location loc, ValueRange ivs) {
          ...
          auto loadedLhs = nestedBuilder.create<AffineLoadOp>(
              loc, gemmAdaptor.getLhs(), ValueRange{i, k});
          auto loadedRhs = nestedBuilder.create<AffineLoadOp>(
              loc, gemmAdaptor.getRhs(), ValueRange{k, j});
          auto product =
              nestedBuilder.create<arith::MulFOp>(loc, loadedLhs, loadedRhs);
          auto current = nestedBuilder.create<AffineLoadOp>(
              loc, alloc, ValueRange{i, j});
          auto sum = nestedBuilder.create<arith::AddFOp>(loc, current, product);
          nestedBuilder.create<AffineStoreOp>(loc, sum, alloc,
                                              ValueRange{i, j});
        });
